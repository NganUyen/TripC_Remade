...
CREATE TABLE public.dining_reservations (
  ...
  CONSTRAINT dining_reservations_booking_id_fkey FOREIGN KEY (booking_id) REFERENCES public.bookings(id),
  CONSTRAINT dining_reservations_venue_id_fkey FOREIGN KEY (venue_id) REFERENCES public.dining_venues(id),
  CONSTRAINT dining_reservations_table_id_fkey FOREIGN KEY (table_id) REFERENCES public.dining_tables(id),
  CONSTRAINT dining_reservations_user_uuid_fkey FOREIGN KEY (user_uuid) REFERENCES public.users(id)
);
CREATE TABLE public.dining_reviews (
  ...
  CONSTRAINT dining_reviews_booking_id_fkey FOREIGN KEY (reservation_id) REFERENCES public.dining_reservations(id)
);
...
CREATE TABLE public.hotel_reviews (
  ...
  cons ARRAY,
  traveler_type text,
  status text DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text, 'flagged'::text])),
  ...
  CONSTRAINT hotel_reviews_booking_id_fkey FOREIGN KEY (booking_id) REFERENCES public.bookings(id),
  CONSTRAINT hotel_reviews_user_uuid_fkey FOREIGN KEY (user_uuid) REFERENCES public.users(id),
  CONSTRAINT hotel_reviews_hotel_id_fkey FOREIGN KEY (hotel_id) REFERENCES public.hotels(id)
);
...
CREATE TABLE public.shop_user_vouchers (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  template_id uuid NOT NULL,
  unique_code character varying NOT NULL UNIQUE,
  status character varying NOT NULL DEFAULT 'active'::character varying CHECK (status::text = ANY (ARRAY['active'::character varying, 'used'::character varying, 'expired'::character varying]::text[])),
  acquired_at timestamp with time zone DEFAULT now(),
  used_at timestamp with time zone,
  expires_at timestamp with time zone NOT NULL,
  used_on_order_id uuid,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT shop_user_vouchers_pkey PRIMARY KEY (id),
  CONSTRAINT shop_user_vouchers_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT shop_user_vouchers_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.voucher_templates(id),
  CONSTRAINT shop_user_vouchers_used_on_order_id_fkey FOREIGN KEY (used_on_order_id) REFERENCES public.shop_orders(id)
);
