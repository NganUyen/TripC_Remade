...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isValidIATACode(origin)) {
      return NextResponse.json(
        { error: 'Invalid origin airport code', message: 'Must be 3-letter IATA code' },
        { status: 400 }
      );
    }
    ...
    if (!isValidIATACode(destination)) {
      return NextResponse.json(
        { error: 'Invalid destination airport code', message: 'Must be 3-letter IATA code' },
        { status: 400 }
      );
    }
    ...
    const filteredOffers = (offers || []).filter(offer => {
      ...
      const matchesRoute = flight.origin === origin && flight.destination === destination;
      ...
      results: {
        ...
        offers: filteredOffers.map(offer => ({
          ...
          flight: {
            ...
            amenities: offer.flights.amenities,
            baggage: offer.flights.baggage_allowance,
