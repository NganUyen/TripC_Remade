 ...
 */
async function getAvailableFilters(supabase: any, currentFilters: { q?: string, category_id?: string | null }) {
  try {
    ...
    const prices = priceRange?.flatMap(p => [p.min_price, p.max_price]).filter(p => p !== null);
