 ...
 *
 * GET /api/entertainment/items/:id/sessions - List sessions with availability
 */
  ...
  try {
    ...
    sessions?.forEach((session) => {
      ...
      sessionsByDate[date].push({
        ...
        availability_percentage: (
          (session.available_count / session.capacity) *
          100
        ).toFixed(0),
        is_selling_fast:
          session.available_count <= item.urgency_threshold || 10,
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        sold_out_sessions:
          sessions?.filter((s) => s.status === "sold_out").length || 0,
      },
    });
  } catch (error: any) {
