    ...
    if (ratingStats && ratingStats.length > 0) {
      ratingStats.forEach((r: any) => {
        ratingDistribution[r.rating as keyof typeof ratingDistribution] = 
          (ratingDistribution[r.rating as keyof typeof ratingDistribution] as number) + 1;
      });
      const sum = ratingStats.reduce((acc, r) => acc + r.rating, 0);
      ratingDistribution.average = Math.round((sum / ratingStats.length) * 10) / 10;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingReview) {
      return NextResponse.json(
        { error: 'You have already reviewed this item. Use PUT to update your review.' },
        { status: 400 }
      );
    }
