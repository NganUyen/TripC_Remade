...
export async function POST(request: NextRequest) {
    try {
        ...
        if (clerkId) {
            userId = await getDbUserId(clerkId);
            if (!userId) {
                return errorResponse('USER_NOT_FOUND', 'User record not found', 404);
            }
        }

        // If no userId, we pass null to the creation logic (nullable UUID column)
        ...
        const sessionKey = clerkId || request.headers.get('x-session-id') || 'GUEST_SESSION';
        const order = await createOrder(finalUserId as any, sessionKey, shipping_address_id, shipping_method_id);
