...
import { requirePartnerAccess, inviteTeamMember, PartnerError } from '@/lib/shop/partner-queries';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const member = await inviteTeamMember(partner.id, userId, body);
        return successResponse(member, undefined, 201);
    } catch (error) {
        if (error instanceof PartnerError) {
            return errorResponse(error.code, error.message, error.status, error.details);
        }
        console.error('Partner team invite error:', error);
        return errorResponse('INTERNAL_ERROR', 'Failed to invite team member', 500);
    }
}
