...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      supabase
        .from("activities")
        .select("id, price, rating, category")
        .eq("partner_id", user.id),
      supabase
        .from("events")
        .select("id, average_rating, category")
        .eq("partner_id", user.id),
      supabase
    ...
    activities?.forEach((a) =>
      itemCategoryMap.set(a.id, a.category || "Activity"),
    );
    ...
    if (organizerIds.length > 0) {
      ...
      if (entItems) {
        ...
        entItems.forEach((e) => {
          const catName =
            entCategoryNameMap.get(e.category_id) || e.type || "Entertainment";
    ...
    events?.forEach((e) => {
      if (e.average_rating) {
        totalRatingSum += e.average_rating;
        totalRatedItems++;
      }
    });
    entertainmentItems.forEach((e) => {
      if (e.rating_average) {
        totalRatingSum += e.rating_average;
        totalRatedItems++;
      }
    });

    const averageRating =
      totalRatedItems > 0 ? totalRatingSum / totalRatedItems : 0;
    const activeListings =
      activityIds.length + eventIds.length + entertainmentIds.length;
    ...
    const [activityBookings, eventBookings, entertainmentBookings] =
      await Promise.all([
    ...
    normalizedBookings.forEach((b) => {
      ...
      if (bookingDate >= startDate) {
        ...
        categoryRevenueMap.set(
          catName,
          (categoryRevenueMap.get(catName) || 0) + amount,
        );
      }
    });
