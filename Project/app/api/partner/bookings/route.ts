...
export async function GET(request: Request) {
  try {
    ...
    const [{ data: activities }, { data: events }, { data: organizers }] =
      await Promise.all([
        supabase
          .from("activities")
          .select("id, title")
    ...
    const normalize = (
      booking: any,
      type: "activity" | "event" | "entertainment",
    ...
    const combinedBookings = [
      ...(activityBookings || []).map((b) =>
        normalize(b, "activity", activityMap, "activity_id"),
      ),
      ...(eventBookings || []).map((b) =>
        normalize(b, "event", eventMap, "event_id"),
      ),
      ...(entertainmentBookings || []).map((b) =>
        normalize(b, "entertainment", entertainmentMap, "entertainment_id"),
      ),
    ];
