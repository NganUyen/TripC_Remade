...
export class HotelSettlementHandler implements ISettlementHandler {
  ...
  async settle(booking: any): Promise<void> {
    console.log(
      "[HOTEL_SETTLEMENT] Starting settlement for booking:",
      booking.id,
    );
    ...
    if (!hotelId || !roomId) {
      console.error(
        "[HOTEL_SETTLEMENT] Critical Error: Missing hotel/room ID in metadata",
      );
      throw new Error("Missing hotel/room ID in metadata");
    }

    console.log(
      "[HOTEL_SETTLEMENT] Resolved internal UUID:",
      userUuid || "GUEST",
    );
    ...
    const { error } = await this.supabase.from("hotel_bookings").insert({
      ...
      room_id: roomId,
      user_uuid: userUuid,
      check_in_date: dates?.start || booking.start_date,
      check_out_date: dates?.end || booking.end_date,
      ...
      guest_phone:
        guestDetails?.phone || booking.guest_details?.phone || "0000000000",
    ...
    console.log(
      "[HOTEL_SETTLEMENT] Successfully created hotel_bookings record",
    );
  }
}
