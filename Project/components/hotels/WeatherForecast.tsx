...
export function WeatherForecast({ latitude, longitude, className = '' }: WeatherForecastProps) {
  ...
  if (loading) {
    return (
      <div className={`bg-blue-50 dark:bg-blue-900/10 rounded-[2rem] p-6 border border-blue-100 dark:border-blue-800/30 ${className}`}>
      ...
      <div className="flex justify-between items-center bg-white/60 dark:bg-black/20 rounded-xl p-4">
        {forecast.map((day, index) => (
          <div key={index} className="text-center">
            ...
            <p className="font-bold text-slate-900 dark:text-white">{day.temp}°</p>
          </div>
        ))}
      </div>
    </div>
