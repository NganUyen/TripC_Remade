...
import { Calendar, Users, ChevronDown, Check, Loader2, CheckCircle, X } from 'lucide-react'
...
export function BookingSidebar({ hotel }: BookingSidebarProps) {
        ...
        <aside className="hidden lg:block w-full sticky top-24 z-30">
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="absolute inset-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm rounded-[2rem] flex items-center justify-center z-50 p-6">
                        <div className="text-center space-y-4">
                            ...
                            <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-xl">
                                <p className="text-sm text-slate-600 dark:text-slate-400 mb-1">Confirmation Code</p>
                                <p className="text-3xl font-black text-orange-600 tracking-wider">{confirmationCode}</p>
                            </div>
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-px bg-slate-100 dark:bg-slate-800 p-1 rounded-[1.5rem]">
                        <div className="bg-white dark:bg-slate-900 p-3 rounded-l-[1.2rem] rounded-r-lg text-left">
                            <label htmlFor="checkin" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">Check-in</label>
                            <input
                                type="date"
                                id="checkin"
                                value={checkInDate}
                        ...
                        <div className="bg-white dark:bg-slate-900 p-3 rounded-r-[1.2rem] rounded-l-lg text-left">
                            <label htmlFor="checkout" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">Check-out</label>
                            <input
                                type="date"
                                id="checkout"
                                value={checkOutDate}
                        ...
                        >
                            <div>
                                ...
                                <p className="font-bold text-slate-900 dark:text-white">
                                    {adults} Adult{adults !== 1 ? 's' : ''}{children > 0 ? `, ${children} Child${children !== 1 ? 'ren' : ''}` : ''}, 1 Room
                                </p>
                            </div>
                            <ChevronDown className={`w-5 h-5 text-slate-400 group-hover:text-orange-500 transition-all ${showGuestDropdown ? 'rotate-180' : ''}`} />
                        </button>
                        ...
                        {showGuestDropdown && (
                            <div className="absolute top-full mt-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl p-4 space-y-3 z-10">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Adults</span>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="font-bold text-slate-900 dark:text-white w-6 text-center">{adults}</span>
                                        <button
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="font-bold text-slate-900 dark:text-white w-6 text-center">{children}</span>
                                        <button
                ...
                {nights > 0 && (
                    <div className="mb-4 space-y-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                        <div className="flex justify-between text-sm">
                            <span className="text-slate-600 dark:text-slate-400">${price} × {nights} night{nights !== 1 ? 's' : ''}</span>
                            <span className="font-bold text-slate-900 dark:text-white">${roomTotal.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex justify-between">
                            ...
                            <span className="font-black text-xl text-orange-600">${grandTotal.toFixed(2)}</span>
                        </div>
                    </div>
                )}
                ...
                <div className="flex items-center justify-center gap-2 text-slate-500 text-xs font-medium">
                    <Check className="w-3 h-3 text-green-500" /> Free cancellation before check-in
                </div>
