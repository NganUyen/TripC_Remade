        ...
        <div className="space-y-12">
            ...
            <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { icon: <Wifi className="w-6 h-6 text-slate-900 dark:text-white" />, label: "High-Speed Wifi" },
                        { icon: <Waves className="w-6 h-6 text-blue-500" />, label: "Infinity Pool" },
                        { icon: <Utensils className="w-6 h-6 text-orange-500" />, label: "Gourmet Dining" },
                        { icon: <Wind className="w-6 h-6 text-green-500" />, label: "Luxury Spa" },
                        { icon: <Coffee className="w-6 h-6 text-amber-700" />, label: "24/7 Cafe" },
                        { icon: <Gamepad2 className="w-6 h-6 text-purple-500" />, label: "Kids Club" },
                    ].map((item, i) => (
                        <div key={i} className="aspect-square rounded-[1.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-zinc-800 flex flex-col items-center justify-center gap-3 hover:shadow-lg transition-all group cursor-default">
                            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl group-hover:scale-110 transition-transform">
                                {item.icon}
                            </div>
                            <span className="font-bold text-sm text-slate-600 dark:text-slate-300 text-center px-2">{item.label}</span>
                        </div>
                    ))}
                </div>
            </div>
