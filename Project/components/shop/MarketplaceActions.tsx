...
export function MarketplaceActions() {
    return (
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-fr">
            {/* Added auto-rows-fr to ensure equal height rows if implied, but h-full on children is key */}

            {/* Voucher Banner - Spans 2 Columns */}
            <div className="md:col-span-2 relative h-full min-h-[220px] rounded-[2rem] overflow-hidden group cursor-pointer">
                ...
                <div className="relative z-10 p-8 h-full flex flex-col justify-center items-start">
                    ...
                    <button className="bg-white text-indigo-900 px-5 py-2 rounded-full text-sm font-bold hover:bg-indigo-50 transition-colors flex items-center gap-2 mt-auto">
                        Browse Vouchers <ArrowRight className="w-4 h-4" />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#18181b] rounded-[2rem] p-8 border border-slate-100 dark:border-zinc-800 flex flex-col justify-between relative overflow-hidden h-full min-h-[220px]">
                ...
                <div>
                    ...
                    <h3 className="text-4xl font-black text-[#1c140d] dark:text-white">2 Items</h3>
                    <p className="text-xs text-slate-400 mt-2">Subtotal: <span className="text-[#FF5E1F] font-bold text-base">$434.00</span></p>
                </div>
                <button className="w-full bg-[#1c140d] dark:bg-white text-white dark:text-black py-4 rounded-xl font-bold mt-6 hover:opacity-90 transition-opacity">
                    Checkout Now
                </button>
            </div>
        </section>
