...
import { Calendar, ChevronLeft, ChevronRight, Plane, Clock } from 'lucide-react';
...
export default function FlightSchedule() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'departed': return 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400';
      case 'arrived': return 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400';
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">{flight.route}</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="text-center">
                  <p className="text-sm text-slate-500 dark:text-slate-400">Khởi hành</p>
                ...
                <div className="text-center">
                  <p className="text-sm text-slate-500 dark:text-slate-400">Đến</p>
                  <p className="text-xl font-bold">{flight.arrival}</p>
                </div>
              </div>
              <span className={`px-4 py-2 rounded-full text-sm font-semibold ${getStatusColor(flight.status)}`}>
