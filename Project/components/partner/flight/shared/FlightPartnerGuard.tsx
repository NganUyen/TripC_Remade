...
import { Clock, Ban, ShieldX, Plane } from "lucide-react";
import Link from "next/link";
import { motion } from "framer-motion";

interface FlightPartnerGuardProps {
  children: ReactNode;
  requireApproved?: boolean;
  loadingSkeleton?: ReactNode;
}

// Default loading skeleton - matches the flight portal structure
...
export function FlightPartnerGuard({
  children,
  requireApproved = true,
  loadingSkeleton,
}: FlightPartnerGuardProps) {
  const { isSignedIn, isLoaded: clerkLoaded } = useUser();
  const { partner, isLoading, fetchPartner } = useFlightPartnerStore();
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]">
        ...
        >
          ...
          <p className="text-slate-400 mb-6">
            Please sign in to access the Flight Partner Portal.
          </p>
          <Link
            href="/sign-in?redirect_url=/partner/flight"
  ...
  // Not a flight partner → show CTA to apply
  if (!partner) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]">
        <motion.div
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-white mb-3">
            Become a Flight Partner
          </h2>
          <p className="text-slate-400 mb-8 leading-relaxed">
            List your airline on TripC and connect with travellers worldwide.
            Manage flights, routes, pricing, and bookings — all from one portal.
          </p>
          <Link
            href="/partner/flight/onboarding"
            className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-sky-500 text-white font-bold text-sm hover:bg-sky-600 transition-colors shadow-lg shadow-sky-500/20"
          >
            Apply Now
          </Link>
        </motion.div>
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]">
        ...
        >
          ...
          <p className="text-slate-400 mb-4 leading-relaxed">
            Your flight partner application for{" "}
            <strong className="text-slate-200">{partner.name}</strong> (
            {partner.airline_code}) is being reviewed. We&apos;ll notify you
            once it&apos;s approved.
          </p>
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]">
        ...
        >
          ...
          <p className="text-slate-400 mb-2">
            Your flight partner account has been suspended.
          </p>
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]">
        ...
        >
          ...
          <p className="text-slate-400">
            Your flight partner account has been permanently banned.
          </p>
        </motion.div>
      </div>
    );
