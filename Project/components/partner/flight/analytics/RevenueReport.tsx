      ...
      >
        <div className="bg-gradient-to-br from-primary to-primary/70 text-white rounded-2xl p-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm bg-white/20 px-2 py-1 rounded-full">+{currentData.growth}%</span>
          </div>
          <p className="text-3xl font-bold mb-1">{(currentData.total / 1000000000).toFixed(1)}B ₫</p>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
          ...
          <p className="text-3xl font-bold mb-1">{(currentData.avgPerFlight / 1000000).toFixed(1)}M ₫</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">Trung bình/chuyến</p>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between mb-2">
            <Calendar className="w-8 h-8 text-blue-600" />
          </div>
          <p className="text-3xl font-bold mb-1 text-green-600">+{currentData.growth}%</p>
          ...
          {breakdownData.map((item, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold w-32 text-right">{(item.amount / 1000000000).toFixed(1)}B ₫</span>
              </div>
            </motion.div>
          ))}
        </div>
