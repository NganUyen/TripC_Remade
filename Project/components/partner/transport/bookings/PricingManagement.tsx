...
export function PricingManagement() {
  ...
  const fetchPrices = async () => {
    try {
      ...
      if (error) {
        ...
        const uniqueRoutes = formattedData.reduce(
          (acc: RoutePrice[], route) => {
            const key = `${route.origin}-${route.destination}-${route.type}-${route.vehicle_type}`;
            const exists = acc.find(
              (r) =>
                `${r.origin}-${r.destination}-${r.type}-${r.vehicle_type}` ===
                key,
            );
            if (!exists) {
              acc.push(route);
  ...
  const averagePrices = prices.reduce(
    (acc: Record<string, { total: number; count: number }>, route) => {
      if (!acc[route.type]) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {Object.entries(averagePrices).map(([type, data]) => (
          <div
            key={type}
            className="group bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all"
          >
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
                  ...
                  <th className="text-right px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">
                    Thao tác
                  </th>
                </tr>
              </thead>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <ul className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
              ...
              <li>
                • Giá limousine thường cao hơn 50-100% so với xe buýt thường
              </li>
