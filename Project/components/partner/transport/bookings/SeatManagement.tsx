...
export function SeatManagement() {
    ...
    const getOccupancyColor = (available: number, total: number) => {
        ...
        if (occupancy >= 70) return 'text-amber-600 bg-amber-100 dark:bg-amber-900/20'
    ...
    const totalSeats = routes.reduce((sum, r) => sum + getSeatCapacity(r.vehicle_type), 0)
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: 'Tổng số chuyến', value: routes.length, icon: MapPin, color: 'text-primary', bg: 'bg-primary/10' },
                    { label: 'Tổng số ghế', value: totalSeats, icon: Armchair, color: 'text-blue-600', bg: 'bg-blue-600/10' },
                    { label: 'Ghế trống', value: availableSeats, icon: Users, color: 'text-green-600', bg: 'bg-green-600/10' },
                    { label: 'Tỷ lệ lấp đầy', value: `${occupancyRate}%`, icon: Check, color: 'text-amber-600', bg: 'bg-amber-600/10' }
                ].map((stat, idx) => (
                    <motion.div
                        key={stat.label}
                            ...
                            >
                                ...
                                <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${getOccupancyColor(route.seats_available, capacity)}`}>
                                ...
                                <div className="flex items-center gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                                    <button
                                        onClick={() => updateSeats(route.id, Math.max(0, route.seats_available - 1))}
                                    ...
                                    <button
                                        onClick={() => updateSeats(route.id, Math.min(capacity, route.seats_available + 1))}
