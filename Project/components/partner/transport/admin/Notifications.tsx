...
export function Notifications() {
    ...
    const getTypeConfig = (type: Notification['type']) => {
        const configs = {
            info: { icon: Info, bg: 'bg-blue-50 dark:bg-blue-500/10', text: 'text-blue-600', border: 'border-blue-100 dark:border-blue-500/20', accent: 'bg-blue-600' },
            success: { icon: CheckCircle, bg: 'bg-emerald-50 dark:bg-emerald-500/10', text: 'text-emerald-600', border: 'border-emerald-100 dark:border-emerald-500/20', accent: 'bg-emerald-600' },
            warning: { icon: AlertCircle, bg: 'bg-amber-50 dark:bg-amber-500/10', text: 'text-amber-600', border: 'border-amber-100 dark:border-amber-500/20', accent: 'bg-amber-600' },
            error: { icon: X, bg: 'bg-rose-50 dark:bg-rose-500/10', text: 'text-rose-600', border: 'border-rose-100 dark:border-rose-500/20', accent: 'bg-rose-600' }
        }
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium">
                            {unreadCount > 0 ? `Bạn có ${unreadCount} thông báo mới chưa xem` : 'Bạn đã xem hết tất cả thông báo'}
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    <div className="space-y-4">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="h-28 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 animate-pulse" />
                        ))}
                    </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-6">
                                    <div className={`w-14 h-14 ${config.bg} rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
                                        <config.icon className={`w-7 h-7 ${config.text}`} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                <h3 className={`text-lg font-black tracking-tight ${notification.is_read ? 'text-slate-600 dark:text-slate-400' : 'text-slate-900 dark:text-white'}`}>
                                                    {notification.title}
                                                </h3>
                                        ...
                                        <div className="flex items-center gap-3 mt-3">
                                            <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{formatTime(notification.created_at)}</span>
                                            </div>
                                            {!notification.is_read && (
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-20 border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium max-w-xs mx-auto">
                            {filter === 'unread' ? 'Tuyệt vời! Bạn đã xử lý hết các thông báo quan trọng.' : 'Bạn chưa có thông báo nào trong danh sách này.'}
                        </p>
                    </div>
                )}
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden relative">
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {[
                            { label: 'Đặt chỗ mới', description: 'Thông báo tức thì khi có đơn đặt chỗ mới', enabled: true },
                            { label: 'Giao dịch', description: 'Báo cáo doanh thu và trạng thái thanh toán', enabled: true },
                            { label: 'Phản hồi', description: 'Cập nhật đánh giá và bình luận mới', enabled: true },
                            { label: 'Hệ thống', description: 'Thông báo bảo trì và cập nhật tính năng', enabled: false }
                        ].map((setting) => (
                            <div key={setting.label} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/40 rounded-3xl border border-slate-100 dark:border-slate-700/50 hover:bg-white dark:hover:bg-slate-800 transition-all group/setting">
                                <div>
                                    <p className="font-black text-slate-900 dark:text-white group-hover/setting:text-primary transition-colors">{setting.label}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">{setting.description}</p>
                                </div>
