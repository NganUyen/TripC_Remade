...
export function TransportDashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            routes?.forEach(route => {
                ...
                else if (type.includes('limousine')) capacity = 9 // Default limousine
                else if (type.includes('bus')) capacity = 45 // Default bus
    ...
    const quickActions = [
        { label: 'Tạo tuyến mới', icon: Route, path: '/partner/transport/operations' },
        { label: 'Quản lý đặt chỗ', icon: Ticket, path: '/partner/transport/bookings' },
        { label: 'Cập nhật giá', icon: DollarSign, path: '/partner/transport/operations' },
        { label: 'Xem báo cáo', icon: TrendingUp, path: '/partner/transport/reports' }
    ]
                                ...
                                >
                                    <div>
                                        <p className="font-semibold text-slate-900 dark:text-white text-sm">
                                            {booking.passenger_info?.name || booking.booking_code || 'N/A'}
                                        </p>
