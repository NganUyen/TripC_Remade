...
export function DriverManagement() {
    ...
    const getStatusBadge = (status: Driver['status']) => {
        const config = {
            available: { bg: 'bg-green-100 dark:bg-green-900/20', text: 'text-green-700 dark:text-green-400', label: 'Sẵn sàng', icon: CheckCircle },
            on_trip: { bg: 'bg-blue-100 dark:bg-blue-900/20', text: 'text-blue-700 dark:text-blue-400', label: 'Đang chạy', icon: Car },
            off_duty: { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-600 dark:text-slate-400', label: 'Nghỉ', icon: Clock }
        }
    ...
    return (
        <div className="space-y-6">
                    ...
                    {filteredDrivers.map((driver, index) => (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                                ...
                                <div className="text-center">
                                    <p className="font-bold text-slate-900 dark:text-white">{driver.totalTrips}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-400">Chuyến đi</p>
                                </div>
                            </div>

                            {/* License Expiry */}
                            <div className="text-sm mb-4">
                                <span className="text-slate-500 dark:text-slate-400">Bằng lái hết hạn: </span>
                                <span className={`font-medium ${new Date(driver.licenseExpiry) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000) ? 'text-amber-600' : 'text-slate-900 dark:text-white'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                    { label: 'Tổng tài xế', value: drivers.length, color: 'text-primary' },
                    { label: 'Sẵn sàng', value: drivers.filter(d => d.status === 'available').length, color: 'text-green-600' },
                    { label: 'Đang chạy', value: drivers.filter(d => d.status === 'on_trip').length, color: 'text-blue-600' },
                    { label: 'Nghỉ', value: drivers.filter(d => d.status === 'off_duty').length, color: 'text-slate-600' }
                ].map((stat) => (
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">Thông tin chi tiết đội ngũ lái xe</p>
                                </div>
                            </div>
                            <button
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
                            ...
                            {providers.length > 1 && (
                                <div className="space-y-1.5">
                                    <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Nhà cung cấp</label>
                                    <select
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Số bằng lái</label>
                                        <input
                                            required
                                            type="text"
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Trạng thái công việc</label>
                                    <select
                            ...
                            <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/20 rounded-xl">
                                <p className="text-xs text-amber-700 dark:text-amber-400 italic">
                                    Lưu ý: Thông tin bằng lái cần chính xác để đảm bảo an toàn vận hành và đối soát bảo hiểm.
                                </p>
                            </div>
                        </form>
