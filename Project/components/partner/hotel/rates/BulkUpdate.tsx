...
export function BulkUpdate() {
  ...
  const [updateType, setUpdateType] = useState<'price' | 'availability'>('price');
  ...
  const roomTypes = ['Deluxe Room', 'Suite Room', 'Family Room', 'Presidential Suite'];
  ...
  const handleBulkUpdate = () => {
    ...
    alert(`Cập nhật ${selectedRooms.length} loại phòng từ ${startDate} đến ${endDate}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
        <div className="space-y-6">
          ...
          {/* Update Type */}
          <div>
            <label className="block text-sm font-medium mb-2">Loại cập nhật</label>
            <div className="grid grid-cols-2 gap-4">
              <button
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  ...
                  onChange={(e) => setPriceChange(prev => ({ ...prev, type: e.target.value as any }))}
                ...
                <input
                  ...
                  onChange={(e) => setPriceChange(prev => ({ ...prev, value: parseFloat(e.target.value) }))}
                  placeholder={priceChange.type === 'percentage' ? 'VD: 10' : 'VD: 100000'}
                  className="px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                />
              </div>
            </motion.div>
          ...
          {/* Room Selection */}
          <div>
            <label className="block text-sm font-medium mb-2">Chọn loại phòng</label>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 dark:text-amber-200">
              Thay đổi sẽ áp dụng cho tất cả các ngày trong khoảng thời gian đã chọn.
            </p>
          </div>
        </div>
      </div>
