...
export function DatePicker({
    ...
    const buttonVariant = isInline || variant === "ghost" ? "ghost" : variant as "default" | "outline" | "ghost"

    return (
        <div className={cn("flex flex-col gap-1.5", className)}>
            {label && <Label className="text-xs font-bold text-slate-500 uppercase tracking-wider px-1">{label}</Label>}
                ...
                <PopoverTrigger asChild>
                    <Button
                        ...
                        className={cn(
                            ...
                            isInline && "bg-transparent border-none shadow-none p-0 h-auto hover:bg-transparent justify-start gap-2"
                        )}
                    >
                        <div className="flex items-center gap-2 truncate">
                            <CalendarIcon className={cn("h-4 w-4 shrink-0", isInline ? "text-slate-400 group-hover:text-[#FF5E1F]" : "opacity-50")} />
                            <span className={cn("truncate", isInline && "text-base font-bold text-slate-900 dark:text-white")}>
                                {date ? format(date, "dd/MM/yyyy") : placeholder}
                            </span>
                        </div>
                        {!isInline && <ChevronDown className="h-4 w-4 opacity-50 shrink-0" />}
                    </Button>
                </PopoverTrigger>
