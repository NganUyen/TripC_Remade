...
export default function QuickAccessLinks() {
    const handleAIClick = () => {
        ...
        const chatButton = document.querySelector('[data-chat-widget-trigger]') as HTMLElement;
        if (chatButton) {
            chatButton.click();
        } else {
            // Fallback: dispatch a custom event that the ChatWidget can listen to
    ...
    return (
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
            ...
            <div onClick={handleAIClick} className="bg-white dark:bg-slate-900 rounded-[2rem] p-5 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-5 group cursor-pointer hover:border-purple-400/30 transition-all hover:shadow-md">
